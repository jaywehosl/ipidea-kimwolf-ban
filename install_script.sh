...
# Отключаем интерактивные диалоги apt (чтобы не вылезали окна)
...
echo -n "4/6 [....] Загрузка базы IP Leaseweb (может занять минуту)..."
...
echo -e "\r5/6 [DONE] Фаервол настроен жестко.                    "
...
ping -c 1 -W 1 85.17.70.38 > /dev/null 2>&1 || echo "ТЕСТ: Пинг к Leaseweb заблокирован (Успех)."
echo "-------------------------------------------------------"
